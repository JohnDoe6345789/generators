...
from PyQt6.QtWidgets import (
    QApplication, QGraphicsDropShadowEffect, QLabel, QMainWindow,
    QMessageBox, QPushButton, QVBoxLayout, QWidget, QGridLayout, QMenuBar,
)
...
def make_btn(text: str, fn: Callable[[], None]) -> QPushButton:
    ...
    b.clicked.connect(lambda: _safe_call(fn))  # type: ignore[arg-type]
    return b
...
class MainWindow(QMainWindow):
    ...
    def _icon(self) -> None:
        ico = QIcon(str(Path(__file__).with_name('..')
                         .joinpath('..','assets','icon.svg').resolve()))
